...
SIMULATION_DASHBOARD_PATH = os.getenv('SIMULATION_DASHBOARD_PATH', '/api/dashboard/mca').strip() or '/api/dashboard/mca'
...
UPSTREAM_TIMEOUT_SECONDS = max(30.0, float(os.getenv('UPSTREAM_TIMEOUT_SECONDS', '100')))
...
SMTP_FROM_NAME = os.getenv('SMTP_FROM_NAME', '2027 Member of County Assembly Simulation Results').strip()
...
def candidate_constituency(record):
    record = record or {}
    for key in ('constituency', 'constituency_name', 'candidate_constituency', 'home_constituency', 'selected_constituency'):
        if record.get(key):
            return friendly(record.get(key))
    for container in ('location', 'electoral_area', 'geography'):
        nested = record.get(container)
        if isinstance(nested, dict):
            value = candidate_constituency(nested)
            if value:
                return value
    return ''


def candidate_ward(record):
    record = record or {}
    for key in ('ward', 'ward_name', 'candidate_ward', 'home_ward', 'selected_ward'):
        if record.get(key):
            return friendly(record.get(key))
    for container in ('location', 'electoral_area', 'geography'):
        nested = record.get(container)
        if isinstance(nested, dict):
            value = candidate_ward(nested)
            if value:
                return value
    return ''
...
def fetch_snapshot(force=False):
    ...
    with _fetch_lock:
        ...
        try:
            paths = []
            for path in (SIMULATION_DASHBOARD_PATH, '/api/dashboard/mca', '/api/dashboard/member-county-assembly'):
            ...
            if data is None:
                raise RuntimeError('MCA simulation API unavailable (' + '; '.join(errors) + ')')
...
def build_summary(county='', constituency='', ward=''):
    ...
    for c in snap.get('candidates') or []:
        ...
        if cid:
            ...
            candidate_constituencies[cid] = candidate_constituency(c)
            candidate_wards[cid] = candidate_ward(c)
    ...
    for row in stream_rows:
        ...
        constituencies = row.get('candidate_constituencies') or {}
        wards = row.get('candidate_wards') or {}
        for cid, n in (row.get('candidate_votes') or {}).items():
            ...
            candidate_constituencies[cid] = friendly(constituencies.get(cid)) or candidate_constituencies.get(cid, '') or friendly(row.get('constituency'))
            candidate_wards[cid] = friendly(wards.get(cid)) or candidate_wards.get(cid, '') or friendly(row.get('ward'))
    ...
    if not county and not constituency and not ward:
        ...
        if upstream_total or upstream_skipped or upstream_participants:
            ...
            for c in snap.get('candidates') or []:
                ...
                if cid:
                    candidate_names[cid] = c.get('name') or cid
                    candidate_counties[cid] = candidate_county(c) or candidate_counties.get(cid, '')
                    candidate_constituencies[cid] = candidate_constituency(c) or candidate_constituencies.get(cid, '')
                    candidate_wards[cid] = candidate_ward(c) or candidate_wards.get(cid, '')
    ...
    for cid in set(candidate_names) | set(candidate_votes):
        ...
        registered_county = candidate_counties.get(cid, '')
        registered_constituency = candidate_constituencies.get(cid, '')
        registered_ward = candidate_wards.get(cid, '')
        # An MCA candidate is eligible in one ward only.
        if county and (not registered_county or norm(registered_county) != norm(county)):
            continue
        if constituency and (not registered_constituency or norm(registered_constituency) != norm(constituency)):
            continue
        if ward and (not registered_ward or norm(registered_ward) != norm(ward)):
            continue
        candidates.append({
            'candidate_id': cid,
            'candidate': candidate_names.get(cid, cid),
            'county': registered_county or 'County Not Provided',
            'constituency': registered_constituency or 'Constituency Not Provided',
            'ward': registered_ward or 'Ward Not Provided',
            'votes': votes,
...
def build_results_pdf(summary):
    ...
    doc = SimpleDocTemplate(
        ...
        title='Member of County Assembly Candidate Results - Training Simulation Only'
    )
    ...
    story = [
        Paragraph('2027 Member of County Assembly Simulation Results', title_style),
    ...
    data = [['Rank', 'Candidate', 'County', 'Constituency', 'Ward', 'Votes', 'Share']]
    ...
    if candidates:
        for i, row in enumerate(candidates, 1):
            data.append([
                ...
                Paragraph(escape(str(row.get('constituency') or 'Constituency Not Provided')), styles['Normal']),
                Paragraph(escape(str(row.get('ward') or 'Ward Not Provided')), styles['Normal']),
                f"{to_int(row.get('votes')):,}",
                f"{row.get('share', 0)}%",
            ])
    else:
        data.append(['', 'No MCA candidate selections yet.', '', '', '', '0', '0%'])
    table = Table(data, colWidths=[10*mm, 42*mm, 27*mm, 34*mm, 34*mm, 18*mm, 13*mm], repeatRows=1)
...
def send_results_email(recipient, pdf_bytes, summary):
    ...
    candidate_lines = [
        ...
        f"{row.get('constituency') or 'Constituency Not Provided'} / "
        f"{row.get('ward') or 'Ward Not Provided'}: "
        f"{to_int(row.get('votes')):,} votes ({row.get('share', 0)}%)"
        for i, row in enumerate(summary.get('candidates') or [], 1)
    ]
    candidate_results = '\n'.join(candidate_lines) or 'No MCA candidate selections yet.'
    msg = EmailMessage()
    subject_area = ward if ward != 'All Wards' else (constituency if constituency != 'All Constituencies' else county)
    msg['Subject'] = f'MCA Simulation Candidate Results — {subject_area}'
    msg['From'] = f'{SMTP_FROM_NAME} <{SMTP_FROM_EMAIL}>' if SMTP_FROM_NAME else SMTP_FROM_EMAIL
    msg['To'] = recipient
    msg.set_content(
        'Attached are the current 2027 Member of County Assembly Simulation Candidate Results.\n\n'
    ...
    safe_area = re.sub(r'[^A-Za-z0-9_-]+', '_', subject_area).strip('_') or 'All_Areas'
    msg.add_attachment(pdf_bytes, maintype='application', subtype='pdf', filename=f'MCA_Simulation_Results_{safe_area}.pdf')
    if SMTP_USE_SSL:
...
def health():
    ...
    return jsonify({'ok': True, 'service': 'member of county assembly-simulation-dashboard-fresh'})
